...
const deleteAlertNotificationController = async (req, res, next) => {
  try {
    ...
    const deletedAlertNotification = await AlertNotification.findOne({_id : id});
...
const getAllAlertNotificationsController = async (req, res, next) => {
    try {
      ...
      res.status(200).json({ message: 'Alert notifications retrieved successfully!',
  ...
  const getAlertNotificationsByUserTypeController = async (req, res, next) => {
    try {
      ...
      if (userType === 'merchant') {
        ...
        return next(appError('Invalid user type specified. Please specify either merchant, agent, or customer.', 400));
      }
  
      // Fetch alert notifications by user type from the database
      const alertNotifications = await AlertNotification.find(query);
  
      res.status(200).json({ message: 'Alert notifications retrieved successfully!', alertNotifications });
    } catch (err) {
      next(appError(err.message));
    }
  };
